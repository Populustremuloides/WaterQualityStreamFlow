...
for nneighbors in numNeighbors:
    for repeat in range(0, numRepeats):
        ...
        for target in targets:
            ...
            model = KNeighborsRegressor(n_neighbors=nneighbors)
            model.fit(trainX, trainY)
            score = model.score(X[testIndices], y[testIndices])
            dataDict[target].append(score)

df = pd.DataFrame.from_dict(dataDict)
df.to_csv("minmax_prediction_results_KNN_numNeighbors.csv", index=False)
